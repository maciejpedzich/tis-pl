...
(function () {
  ...
  doc[addEventListener](keydown, function (e) {
    nextCodeChar =
      e[keyCode] == "&&((%'%'BA"[charCodeAt](nextCodeChar)
        ? nextCodeChar + 1
        : 0;
    if (!state && nextCodeChar > 9) {
      ...
      (function () {
        var win = window,
          ...
          // 3 2 1 0 <- second byte
          shapes = atob(
            '8ABERAAPIiJxACYCcAQiA3QAIgZwASMCZgBmAGYAZgA2AGIEYAMxAnIAYgJwAjICYwBkAjAGMgE'
          ),
          ...
          html =
            ...
            divStyleMargin +
            '1pc 2pc;color:#888">' +
            '<b><a href="http://github.com/maciejpedzich/tis-pl" style="color:inherit">Tis</a></b>: Klon tetrisa w 4 kB języka JavaScript<br><br>' +
            'Lewo/Prawo: Rusz blokiem | Góra/Ctrl/Alt: Obróć | Esc: Wyjdź<br>' +
            'Dół/Spacja: Przyspiesz/Upuść | M: Włącz/Wyłącz muzykę' +
            divEnd +
            divStyleMargin +
            '0 1pc;float:right;color:#eee;font-size:1pc">' +
            '<div id="tis-status">' +
            divEnd +
            'Następny' +
            divStyleMargin +
        ...
        tmp2 =
          divStyleMargin +
          '0;width:1pc;height:1pc;float:left;box-shadow:-2px -2px 8px rgba(0,0,0,.4) inset,0 0 2px #000 inset" id="tis-';
        for (i = 220; i < s; i++) {
          if (i % w < 4) {
            html += tmp2 + i + '">' + divEnd;
          }
        ...
        html +=
          ...
          divEnd +
          divStyleMargin +
          '0 2pc 2pc;background:#000;width:10pc;height:20pc">';
        ...
        for (delta in music) {
          ...
          for (i = 0, j = 0; j < music[delta].length; ) {
            ...
            x =
              0.00499 * math.pow(2, ((tmp3 % 24) + (delta == 2 ? 0 : 27)) / 12);
            tmp2 = [15, 9, 9][delta];
        ...
        function playSoundEffect(encoding) {
          if (!(tmp5 = sounds[encoding])) {
            ...
            for (j in tmp5) {
              ...
              tmp5[j] =
                127 +
                (tmp4 *= 1 - (encoding & 15) / 1e4) *
                  ((j / 10) % tmp3 < tmp3 / 2 ? -1 : 1);
            }
        ...
        function isSolidAt(x, y, rotation, tetromino) {
          return (
            currentTetromino &&
            !(x & ~3) &&
            !(y & ~3) && // range check for [0, 4)
            shapes[charCodeAt](
              8 * (tetromino || currentTetromino) - 8 + 2 * rotation + (y >> 1)
            ) &
              (1 << (4 * (y & 1) + x))
          );
        }
        ...
        function render() {
          ...
          while (
            currentTetromino &&
            tryMove(currentX, currentY + 1, currentRotation, 1)
          );
          for (y = 0; y < h + 4; y++) {
            for (x = 0; x < w; x++) {
              i = y * w + x;
              ...
              shadowGrid[i] = isSolidAt(x - currentX, y - tmp, currentRotation)
                ? currentTetromino
                : isSolidAt(x - currentX, y - currentY, currentRotation)
                ? currentTetromino + 8
                : grid[i] || 0;
              if ((tmp3 = win['tis-' + i])) {
                tmp3 = tmp3.style;
        ...
        function frame(now) {
          ...
          if (state == 2) {
            ...
            // state == 3: Regular gameplay
            // Handle keyboard input
            for (tmp2 in keysPressed) {
              if (tmp2 == 37 || tmp2 == 39) {
                // Move
                if (keysPressed[tmp2] >= 0) {
                  keysPressed[tmp2] -= keysPressed[tmp2] ? 0.05 : 0.2;
                  tryMove(
                    currentX + 1 - 2 * (tmp2 == 37),
                    currentY,
                    currentRotation
                  );
                }
                ...
                if (!keysPressed[tmp2]) {
                  for (i = 0; i < 5; ) {
                    tmp =
                      (currentTetromino == 1
                        ? wallKickTableI
                        : wallKickTableRest)[charCodeAt](
                        ((currentRotation + 4 + (tmp4 - 1) / 2) % 4) * 5 + i++
                      ) - 32;
                    if (
                      tryMove(
                        currentX + tmp4 * ((tmp & 7) - 2),
                        currentY + tmp4 * (2 - (tmp >> 3)),
                        (currentRotation + 4 + tmp4) % 4
                      )
                    ) {
